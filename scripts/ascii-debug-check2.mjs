...
const chrome = spawn(CHROME, ['--headless=new','--disable-gpu','--enable-unsafe-swiftshader',`--remote-debugging-port=${PORT}`,'--user-data-dir='+join(tmpdir(),'chrome-ascdbg2-'+Date.now()),'--no-first-run','about:blank']);
...
async function check(label, width, height, mobile) {
...
await check('PC   1280x800 (mouse)   ', 1280, 800, false);
await check('Mobile 390x844 (touch)  ', 390, 844, true);
